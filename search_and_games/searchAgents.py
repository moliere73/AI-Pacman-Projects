...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        visitedCorners = (False, False, False, False)  # Initially, no corners have been visited
    ...
    def getResult(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        # Compute the new position after the action
        x,y = state[0]
        dx, dy = Actions.directionToVector(action)
        nextx, nexty = int(x + dx), int(y + dy)
        nextPos = (nextx, nexty)

    # Check if any corners are visited for the first time
        visited = list(state[1])
        for i, corner in enumerate(self.corners):
            if nextPos == corner and not visited[i]:
                visited[i] = True  # Mark this corner as visited

        return (nextPos, tuple(visited))
...
def foodHeuristic(state, problem):
    ...
    if not foodList:
        return 0  # If no food left, the heuristic is zero

    # Use the mazeDistance function to find actual shortest path distances
    def getMazeDistance(pos1, pos2):
        return mazeDistance(pos1, pos2, problem.startingGameState)
        
    # Heuristic: distance to the farthest food pellet
    farthestDistance = max([getMazeDistance(position, food) for food in foodList])
